...
FoldersRoute.post('/:email', async(req,res)=>{
    ...
    try{
        const checkUser = await notesModel.findOne({userEmail : email},{_id:0,userEmail:1,notes:1, folders:1})
        let note = payload.folder
        let checkFolder = checkUser.folders.find(n => n === note )
        if(checkUser && !checkFolder){
            const updateUser = await notesModel.findOneAndUpdate({userEmail: email}, {$push : {"folders" : note }})
            if(updateUser){
                res.send(updateUser)
            }
        }else{
...
FoldersRoute.delete('/:email/:name', async(req,res)=>{
    ...
    try{
        ...
        if(checkUser){
            ...
            let filterNotes = checkUser.notes.filter(note => note.folder !== `/${name}/create` )
            let deleteFolder = await notesModel.findOneAndUpdate({userEmail : email}, {$set:{notes : filterNotes}, $set:{folders : filterFolder}})
